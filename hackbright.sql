...
CREATE TABLE students (
    ...
    girhub character varying(30) NOT NULL
);
...
CREATE VIEW report_card_view AS
 SELECT students.first_name,
    ...
    grades.grade
   FROM ((students
     JOIN grades ON (((students.girhub)::text = (grades.student_github)::text)))
...
3	Maze	Find your way through melon maze	75
4	Shopping Cart	Build a shopping cart for melons	80
5	Terminal Game	Make a python game that runs in terminal	90
\.
...
COPY students (first_name, last_name, girhub) FROM stdin;
Jane	Hacker	jhacks
...
ALTER TABLE ONLY students
    ADD CONSTRAINT students_pkey PRIMARY KEY (girhub);
